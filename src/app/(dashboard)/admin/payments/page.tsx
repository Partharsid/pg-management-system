...
interface Payment {
  ...
  invoice: { id: string; invoiceNumber: string; month: number; year: number } | null;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-gray-900">Payments</h1><p className="text-gray-500">Track rent payments and transactions</p></div>
      ...
      <Card>
        <CardContent className="flex items-center gap-4 p-6">
          ...
          <div><p className="text-sm text-gray-500">Total Collected</p><p className="text-2xl font-bold text-gray-900">{formatCurrency(totalCollected)}</p></div>
        </CardContent>
      </Card>
              ...
              <tbody className="divide-y divide-gray-200">
                {payments.map((payment) => (
                  <tr key={payment.id} className="hover:bg-gray-50">
                    <td className="px-4 py-3"><p className="font-medium text-gray-900">{payment.tenant.name}</p><p className="text-xs text-gray-500">{payment.tenant.phone}</p></td>
                    <td className="px-4 py-3 font-medium text-gray-900">{formatCurrency(Number(payment.amount))}</td>
